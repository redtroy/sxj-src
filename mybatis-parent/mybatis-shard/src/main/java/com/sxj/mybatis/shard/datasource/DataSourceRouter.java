...
public class DataSourceRouter {
	...
	public static DataSource getDataSource(MappedStatement ms,
		...
		if (columnRule == null) {
			...
			// for insert sql
				...
				String insertValues = sql.substring(
						lowerSql.indexOf("values") + 6).trim();
			...
			if (shardValueIndex == -1) {
				...
				Object shardValue = getParamValue(ms, boundSql, param,
						shardValueIndex);
				int value = -1;
				if (shardValue instanceof Integer) {
					...
					value = HashCodeBuilder
							.reflectionHashCode(shardValue, true);
		...
		if (commandType.equals(Command_R)) {
			...
			int index = (int) (targetNode.getWriteNodes().size() * Math
					.random());
	...
	private static Object getParamValue(MappedStatement ms, BoundSql boundSql,
		...
		MetaObject metaObject = param == null ? null : MybatisConfiguration
				.getConfiguration().newMetaObject(param);
