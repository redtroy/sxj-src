...
//        }
        ...
        paramMap.put("itemIds", itemIdsStr);
        List<DocEntity> docList=iDocDao.getDocByItemIds(paramMap);
        List<String> docIds=new ArrayList<String>();
        Map<String,String> windowCodeMap=new HashMap<String,String>();
        for(int i=0;i<docList.size();i++){
            docIds.add(docList.get(i).getId());
            windowCodeMap.put(docList.get(i).getId(), docList.get(i).getWindowCode());
        }
    ...
    public List<DocReportModel> getDocReport(List<String> itemIds)
    {
        ...
        String[] itemIdsStr=itemIds.toArray(new String[itemIds.size()]);
        paramMap.put("itemIds", itemIdsStr);
        List<DocEntity> temDocList=iDocDao.getDocByItemIds(paramMap);
