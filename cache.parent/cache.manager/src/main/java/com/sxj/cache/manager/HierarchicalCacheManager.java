...
public class HierarchicalCacheManager {
	...
	public void initCacheProvider(String configFile, int databaseId,
		...
		final String path = getConfigFile() == null ? CONFIG_FILE
				: getConfigFile();
		InputStream configStream = HierarchicalCacheManager.class
				.getClassLoader().getParent().getResourceAsStream(path);
		...
		try {
			...
			if (props.getProperty("cache.L1.provider_class") != null) {
				this.l1_provider = getProviderInstance(props
			...
			if (props.getProperty("cache.L2.provider_class") != null) {
				HierarchicalCacheManager.l2_provider = getProviderInstance(props
	...
	private final static Cache _GetCache(int level, String cache_name,
		...
		default:
			return new NullCacheProvider().buildCache(cache_name, autoCreate,
					listener);
		}
		// return ((level == 1) ? l1_provider :
		// l2_provider).buildCache(cache_name,
		// autoCreate,
		// listener);
	}
