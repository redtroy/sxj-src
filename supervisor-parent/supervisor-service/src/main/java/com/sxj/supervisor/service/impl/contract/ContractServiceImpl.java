    ...
    public ContractModel getContract(String id) throws ServiceException
    {
        try
        {
            ...
            if (contract != null)
            {
                ...
                List<ContractItemEntity> itemList = contractItemDao.queryItems(contract.getContractNo());// 产品条目
                ...
                List<ContractBatchEntity> batchList = contractBatchDao.queryBacths(batchCondition);// 批次
                if (batchList != null && batchList.size() > 0)
                {
                    ...
                    System.err.println(JsonMapper.nonEmptyMapper()
                            .toJson(bmList));
                ...
                if (modifyRecordIds != null && modifyRecordIds.length() > 0)
                {
                    ...
                    if (modifyList != null && modifyList.size() > 0)
                    {
                        ...
                        for (int i = 0; i < modifyList.size(); i++)
                        {
                            ...
                            List<ModifyItemEntity> item = contractModifyItemDao.queryItems(modify.getId());// 变更条目
                                                    ...
                                                    .length() > 0)
                                    {
                                        ...
                                        try
                                        {
                                            batchItemModel = JsonMapper.nonEmptyMapper()
                                                    .getMapper()
                                                    .readValue(modifyBatchEntity.getBatchItems(),
                                                            new TypeReference<List<BatchItemModel>>()
                                                            {
                                                            });
                                        }
                                                ...
                                                .length() > 0)
                                {
                                    ...
                                    try
                                    {
                                        batchItemModelList = JsonMapper.nonEmptyMapper()
                                                .getMapper()
                                                .readValue(ReplenishBatchEntity.getBatchItems(),
                                                        new TypeReference<List<BatchItemModel>>()
                                                        {
                                                        });
                                    }
    ...
    public ContractReplenishModel getReplenishByRfid(String rfid)
    {
        try
        {
            ...
            if (batchList != null && batchList.size() > 0)
            {
                ...
                bimList = JsonMapper.nonEmptyMapper()
                        .getMapper()
