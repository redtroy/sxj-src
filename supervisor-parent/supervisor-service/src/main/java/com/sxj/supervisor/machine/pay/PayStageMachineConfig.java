...
@StateMachine(stateType = PayStageEnum.class, startState = "STAGE1", finalStates = {
        "STAGE3" }, name = "payStagefsm")
public class PayStageMachineConfig
{
    ...
    @Transitions({
            @Transition(source = "STAGE1", event = "STAGE1_A", target = "STAGE2") })
    ...
    @Transitions({
            @Transition(source = "STAGE2", event = "STAGE2_B", target = "STAGE3") })
