...
public class MemberController extends BaseController {
	...
	public String memberList(MemberQuery query, ModelMap map)
		...
		try {
			...
			if (StringUtils.isNotEmpty(query.getIsDelChangeImage())) {
				CometServiceImpl.setCount(
						MessageChannel.MEMBER_IMAGECHANGE_MESSAGE, 0l);
				CometServiceImpl
						.clear(MessageChannel.MEMBER_IMAGECHANGE_MESSAGE_SET);
	...
	public String info(String id, ModelMap map) throws WebException {
		try {
			 ...
			 * }
			 */
			List<MemberToMemberEntity> relist = mtmservice.queryInfo(member
					.getMemberNo());
	...
	public void exportExcel(HttpServletRequest request,
		...
		InputStream is = MemberController.class.getClassLoader().getResourceAsStream("template/memberInfo.xls"); 
		List<ExportMemberModel> Modelist = memberService
				.queryExportMemberModel(query);
