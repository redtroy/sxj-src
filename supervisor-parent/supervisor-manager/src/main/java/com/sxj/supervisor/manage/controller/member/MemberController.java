...
public class MemberController extends BaseController {
	...
	public String memberList(MemberQuery query, ModelMap map)
		...
		try {
			...
			if (StringUtils.isNotEmpty(query.getIsDelMesPerfect())) {
				...
				CometServiceImpl
						.clear(MessageChannel.MEMBER_PERFECT_MESSAGE_SET);
			}
	...
	public String info(String id, ModelMap map) throws WebException {
		try {
			...
			List<AreaEntity> cityList = areaService.getChildrenAreas("32");
			// 获取证书
			List<MemberImageEntity> ImageList = imageService.getImages(
					member.getMemberNo(), "1");
			// 获取上一版本证书
			List<MemberImageEntity> oldImageList = imageService
					.historyImage(member.getMemberNo());
			 ...
			 * }
			 */
			List<RelevanceMember> relist = memberService
					.getListRelevanceMember(member.getMemberNo());
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				ws.addCell(new Label(8, i + 1,
						member.getRegisteredCapital() == null ? "" : member
								.getRegisteredCapital().toString()));
				ws.addCell(new Label(9, i + 1, DateTimeUtils
						.formatPageDate(member.getFoundedDate())));
				...
				ws.addCell(new Label(13, i + 1, DateTimeUtils
						.formatPageDate(member.getRegDate())));
				ws.addCell(new Label(14, i + 1, DateTimeUtils
						.formatPageDate(member.getAuthorDate())));
	...
	public @ResponseBody Map<String, MemberImageModel> getMemberImage(
		...
		try {
			MemberImageModel model = imageService
					.getMemberImageByImageId(imageId);
			map.put("model", model);
		} catch (Exception e) {
			SxjLogger.error("修改会员信息错误", e, this.getClass());
			throw new WebException(e.getMessage());
