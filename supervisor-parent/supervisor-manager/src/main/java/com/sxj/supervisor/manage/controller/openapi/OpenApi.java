...
public class OpenApi extends BaseController
{
    ...
    public @ResponseBody String developerList(DevelopersEntity query, HttpServletResponse response) throws WebException{
        try{
    ...
    public @ResponseBody String memberList(String name,String type, HttpServletResponse response) throws WebException{
        try{
            List<ApiModel> list = memberService.apiQueryMembers(name,type);
            response.setContentType("application/json; charset=utf-8");
            PrintWriter out = response.getWriter();
            out.print(JsonMapper.nonEmptyMapper().toJson(list));
            out.flush();
