...
public class BasicController extends BaseController {
	...
	public void uEdit(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			InputStream stream = ClassLoaderUtil
			...
			if (request instanceof DefaultMultipartHttpServletRequest) {
				...
				response.getWriter().write(
						new ActionEnter(request, storageClientService,
								fileEntitys.toArray(new FileEntity[fileEntitys
										.size()]), stream).exec());
			} else {
				response.getWriter().write(
						new ActionEnter(request, stream).exec());
			}
	...
	public String toMenu(ModelMap map) {
		...
		List<FunctionModel> list = roleService.getRoleFunction(
				userName.getId(), "");
		map.put("list", list);
		return "manage/menu";
	}
	...
	public @ResponseBody Map<String, String> autoDevelopers(
		...
		List<DevelopersEntity> list = developerService
				.queryDeveloperList(query);
		List<String> strlist = new ArrayList<String>();
		String sb = "";
	...
	public static void main(String... args) {
		try {
			java.net.HttpURLConnection connection = (HttpURLConnection) new URL(
					"http://192.168.1.115:8080/rfid-website/rfid/test")
					.openConnection();
