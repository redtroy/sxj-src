...
public class BasicController extends BaseController {
			...
			if (session == null || session.getAttribute("userinfo") == null) {
				...
				if (info.getAccount() != null && info.getMember() != null) {
					...
					List<AreaEntity> cityList = areaService
							.getChildrenAreas("32");
					MemberEntity member = memberService.getMember(info
							.getMember().getId());
					...
					if (info.getMember().getFlag()) {
						...
						query.setCheckState(MemberCheckStateEnum.CERTIFIED
								.getId());
						...
						query = new MemberQuery();
						query.setMemberType(MemberTypeEnum.GENRESFACTORY
								.getId());
						query.setCheckState(MemberCheckStateEnum.CERTIFIED
								.getId());
						query.setFilterStr(1);
						...
						// CometServiceImpl.get(MessageChannel.MEMBER_TENDER_MESSAGE_INFO);
						// if (infoIdList != null && infoIdList.size() > 0)
						// {
						// List<TenderMessageEntity> messageList = new
						// ArrayList<TenderMessageEntity>();
						// for (Iterator<String> iterator =
						// infoIdList.iterator(); iterator.hasNext();)
						// {
						...
						// List<TenderMessageModel> list =
						// tenderMessageService.queryMessageList(query2);
						// if (list == null || list.size() == 0)
						// {
						// TenderMessageEntity message = new
						// TenderMessageEntity();
						...
						// TenderMessageQuery messQuery = new
						// TenderMessageQuery();
						 ...
						 * System.
						 * err.println("招标:"+messageList.get(i).getInfoId()); }
						 */
		...
		for (MultipartFile myfile : files) {
			if (myfile.isEmpty()) {
				...
				String filePathBack = storageClientService.uploadFile(null,
						myfile.getInputStream(), myfile.getBytes().length,
						extName);
				// 转HTML
				byte[] html = convert2Html(myfile.getInputStream()).getBytes();
				String filePath = storageClientService.uploadFile(null,
						new ByteArrayInputStream(html), html.length, "JPG");
				SxjLogger.info("siteUploadFilePath=" + filePath,
						this.getClass());
				fileIds.add(filePath);
	...
	public @ResponseBody Map<String, Long> menuMessageCount(HttpSession session)
		...
		try {
			...
			Long sysMessageCount = CometServiceImpl
					.getCount(MessageChannel.MEMBER_SYSTEM_MESSAGE_COUNT
							+ member.getMemberNo());
			Long tenderMessageCount = tenderMessageService.queryUnread(member
					.getMemberNo());
	...
	public @ResponseBody Map<String, String> getRefContractWindwoType(
		...
		List<ContractItemEntity> list = contractService.getRefContractItem(
				refContractNo, keyword);
		List strlist = new ArrayList();
		String sb = "";
