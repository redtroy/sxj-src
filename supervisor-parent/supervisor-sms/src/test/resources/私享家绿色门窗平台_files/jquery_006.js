		...
		this.autocomplete = function(param){
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey){
					...
					var offHeight=offset.top+35;
					$bigAutocompleteContent.css({"top":offHeight,"left":offset.left});
					...
					if(data != null && $.isArray(data) ){
						...
						$.post(url,{keyword:keyword_},function(result){
							if($(node).data("config").beforeShow){
								//if(result.data.length>1)
								//	makeContAndShow(result.data);
								//else
									$(node).data("config").beforeShow(result.data);
							}
							//else{
							//	makeContAndShow(result.data);
							//}
