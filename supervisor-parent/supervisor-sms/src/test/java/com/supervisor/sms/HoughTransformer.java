...
public class HoughTransformer
{
                ...
                if (inputData.contrast(x, y, minContrast))
                {
                    for (int theta = thetaAxisSize - 1; theta >= 0; theta--)
                    {
                        ...
                        int rScaled = (int) Math
                                .round(r * halfRAxisSize / maxRadius)
                                + halfRAxisSize;
        ...
        for (int y = 0; y < arrayData.height; y++)
        {
            for (int x = 0; x < arrayData.width; x++)
            {
                int n = Math.min(
                        ...
                        255);
                outputImage.setRGB(x,
                        arrayData.height - 1 - y,
                        (n << 16) | (n << 8) | 0x90 | -0x01000000);
            }
        }
    ...
    {
        ArrayData inputData = getArrayDataFromImage("D://a.png");
        int minContrast = (args.length >= 4) ? 64 : Integer.parseInt("100");
        ArrayData outputData = houghTransform(inputData,
                Integer.parseInt("130"),
                Integer.parseInt("50"),
                minContrast);
        writeOutputImage("D://a1.png", outputData);
        return;
    }
}