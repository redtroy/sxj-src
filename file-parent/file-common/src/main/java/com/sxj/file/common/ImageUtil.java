...
public class ImageUtil {
	...
	public static byte[] scaleRate(byte[] file_buff, int width, int height,
		...
		try {
			...
			if (height == 0) {
				...
				return scaleRate(file_buff, width / w, width / w,
						file_ext_name, null);
			}
			if (width == 0) {
				if (h <= height) {
				...
				return scaleRate(file_buff, height / h, height / h,
						file_ext_name, null);
			}
			if (w / h > width / height) {
				return scaleRate(file_buff, width / w, width / w,
			...
			return scaleRate(file_buff, height / h, height / h, file_ext_name,
					null);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	 ...
	 * @return
	 */
	public static byte[] scaleFixed(byte[] file_buff, int width, int height,
			String file_ext_name, boolean changeRate) {
		try {
			if (file_buff == null) {
				return null;
			}
			...
			Image from = srcImage.getScaledInstance(srcImage.getWidth(),
					srcImage.getHeight(), BufferedImage.SCALE_DEFAULT);
			...
			if (file_ext_name.toLowerCase().equals("png")) {
				toImage = g2d.getDeviceConfiguration().createCompatibleImage(
			...
			int[][][] positions = {
									...
									(height - fontSize) / 2 } },
					{
							...
							{ width - (pressText.length() * fontSize) - bianju,
									height - fontSize - bianju } } };
			if (position == 0) {
				position = NumberUtils.getRandomIntInMax(9) + 1;
			}
			x = positions[((position - 1) / 3)][((position - 1) % 3)][0];
			y = positions[((position - 1) / 3)][((position - 1) % 3)][1];
	...
	public static void main(String[] args) {
		try {
			...
			scaleFixedImageFile(
					...
					"D:/My Documents/My Pictures/dq5nds_001_small2.jpg", 100,
					100, false);
