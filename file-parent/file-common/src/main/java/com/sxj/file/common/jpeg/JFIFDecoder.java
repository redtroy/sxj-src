...
public class JFIFDecoder
{
    ...
    */
    ...
    public JFIFDecoder(ImageInputStream in) throws IOException, JPEGException
    {
            ...
            // information away because we wont be using it.
                ...
                case JPEGMarker.SOF0:
                    ...
                    for (int i = 0; i < frame.getComponentCount(); i++)
                        frame.addComponent(jpegStream.readByte(),
                                jpegStream.readByte(),
                                jpegStream.readByte());
                    break;
                ...
                case JPEGMarker.DQT:
                    ...
                    for (int j = 0; j < quantizationLength / 65; j++)
                    {
                        ...
                        qTables[(int) (quantSpecs & 0x0f)] = new JPEGQTable(
                                quantData);
                    }
                    break;
                case JPEGMarker.SOS:
                        ...
                        catch (JPEGMarkerFoundException bse)
                        {
                                    ...
                                    || marker == JPEGMarker.RST7)
                            {
                                for (int compIndex = 0; compIndex < numberOfComponents; compIndex++)
                                {
                                    JPEGComponent comp = (JPEGComponent) frame.components.getComponentByID(componentSelector[compIndex]);
                                    if (compIndex > 1)
                                        comp.padMCU(mcuTotalIndex,
                                                resetInterval - mcuIndex);
                                    comp.resetInterval();
                                }
                    ...
                    else if (jpegFrames.size() == 1)
                    {
                        ...
                        if (frame.getComponentCount() == 1)
                        {
                            ...
                            ComponentColorModel ccm = new ComponentColorModel(
                                    cs, false, false, Transparency.OPAQUE,
                        ...
                        {
                            ComponentColorModel ccm = new ComponentColorModel(
                                    new YCbCr_ColorSpace(), false, false,
