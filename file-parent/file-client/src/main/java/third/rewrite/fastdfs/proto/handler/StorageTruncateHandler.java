...
public class StorageTruncateHandler extends AbstractHandler<Void> {
	 ...
	 * @param ext
	 */
	public StorageTruncateHandler(Socket socket,  
			String path, long size , Charset charset) {
		super(socket);
		this.size = size;
	...
	protected void send(OutputStream ous) throws IOException {
		...
		body_len = 2 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE
				+ pathBytes.length;
		...
		System.arraycopy(pathBytes, 0, wholePkg, offset,
				pathBytes.length);
