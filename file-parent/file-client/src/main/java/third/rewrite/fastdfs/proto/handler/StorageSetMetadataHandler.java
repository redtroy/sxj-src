...
public class StorageSetMetadataHandler extends AbstractHandler<Void> {
	 ...
	 * @param ext
	 */
	public StorageSetMetadataHandler(Socket socket, String groupName,
			String path, NameValuePair[] metaList, byte opFlag, Charset charset) {
		super(socket);
	...
	protected void send(OutputStream ous) throws IOException {
		...
		System.arraycopy(bs, 0, sizeBytes,
				OtherConstants.FDFS_PROTO_PKG_LEN_SIZE, bs.length);
		...
		header = packHeader(cmd, 2 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE + 1
				+ groupBytes.length + filenameBytes.length + meta_buff.length);
