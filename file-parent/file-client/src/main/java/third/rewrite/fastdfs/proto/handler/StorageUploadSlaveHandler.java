...
public class StorageUploadSlaveHandler extends AbstractHandler<StorePath> {
	 ...
	 * @param ext
	 */
	public StorageUploadSlaveHandler(Socket socket, InputStream ins, long size,
			String masterFilename, String prefixName, String ext,
			Charset charset) {
		super(socket);
		this.ins = ins;
		this.size = size;
	...
	protected void send(OutputStream ous) throws IOException {
		...
		body_len = sizeBytes.length + OtherConstants.FDFS_FILE_PREFIX_MAX_LEN
				+ OtherConstants.FDFS_FILE_EXT_NAME_MAX_LEN
				+ masterFilenameBytes.length + size;
