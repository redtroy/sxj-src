...
public class StorageModifyHandler extends AbstractHandler<Void> {
	 ...
	 * @param ext
	 */
	public StorageModifyHandler(Socket socket, InputStream ins, long size,
			String path, long fileOffset, Charset charset) {
		super(socket);
		this.ins = ins;
		this.size = size;
	...
	protected void send(OutputStream ous) throws IOException {
		...
		body_len = 3 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE
				+ pathBytes.length + size;
