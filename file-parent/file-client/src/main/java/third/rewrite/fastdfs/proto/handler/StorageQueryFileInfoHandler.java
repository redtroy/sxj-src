...
public class StorageQueryFileInfoHandler extends AbstractHandler<FileInfo> {
	 ...
	 * @param charset
	 */
	public StorageQueryFileInfoHandler(Socket socket, String groupName,
	...
	protected void receive(InputStream ins) throws IOException {
		...
		if (3 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE
				+ OtherConstants.FDFS_IPADDR_SIZE != contentLength) {
	...
	private FileInfo decode(byte[] bs) {
		...
		int create_timestamp = (int) BytesUtil.buff2long(bs,
				OtherConstants.FDFS_PROTO_PKG_LEN_SIZE);
		int crc32 = (int) BytesUtil.buff2long(bs,
				2 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE);
		String source_ip_addr = (new String(bs,
				3 * OtherConstants.FDFS_PROTO_PKG_LEN_SIZE,
