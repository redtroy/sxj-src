 ...
 *
 */
public class RedisLock extends RedisObject implements RLock
{
    ...
    private static final ConcurrentMap<String, RedisLockEntry> ENTRIES = new ConcurrentHashMap<String, RedisLockEntry>();
    
    public RedisLock(ConnectionManager connectionManager, String name, UUID id)
    {
    ...
    private void unsubscribe()
    {
        while (true)
        {
            RedisLockEntry entry = ENTRIES.get(getEntryName());
            if (entry == null)
            {
                return;
            }
            RedisLockEntry newEntry = new RedisLockEntry(entry);
            newEntry.release();
    ...
    private Future<Boolean> subscribe()
    {
        ...
        final RedisLockEntry value = new RedisLockEntry(newPromise);
        value.aquire();
        RedisLockEntry oldValue = ENTRIES.putIfAbsent(getEntryName(), value);
        if (oldValue != null)
        {
    ...
    {
        final LockValue currentLock = new LockValue(id, Thread.currentThread()
                .getId());
        currentLock.incCounter();
        
        return getConnectionManager().write(new SyncOperation<LockValue, Long>()
        {
    ...
    private Long tryLockInner(final long leaseTime, final TimeUnit unit)
    {
        ...
        return getConnectionManager().write(new SyncOperation<Object, Long>()
        {
            @Override
            public Long execute(RedisConnection<Object, Object> connection)
            {
                Boolean setnx = connection.setnx(getName(), currentLock);
    ...
    public void unlock()
    {
        getConnectionManager().write(new SyncOperation<Object, Void>()
        {
            ...
            public Void execute(RedisConnection<Object, Object> connection)
            {
                ...
                if (lock != null)
                {
                    LockValue currentLock = new LockValue(id,
    ...
    {
        LockValue lock = getConnectionManager().read(new ResultOperation<LockValue, LockValue>()
        {
            @Override
            protected Future<LockValue> execute(
                    RedisAsyncConnection<Object, LockValue> async)
            {
