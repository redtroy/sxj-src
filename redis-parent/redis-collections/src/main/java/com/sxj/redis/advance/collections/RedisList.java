...
public class RedisList<V> extends RedisExpirable implements RList<V>
{
    ...
    public RedisList(ConnectionManager connectionManager, String name)
    {
    ...
    public boolean containsAll(Collection<?> c)
    {
        ...
        {
            final int j = i;
            List<Object> range = getConnectionManager().read(new ResultOperation<List<Object>, Object>()
            {
                @Override
                protected Future<List<Object>> execute(
                        RedisAsyncConnection<Object, Object> async)
                {
                    return async.lrange(getName(), j * batchSize, j * batchSize
                            + batchSize - 1);
                }
            });
    ...
    public void clear()
    {
        getConnectionManager().write(new ResultOperation<Long, V>()
        {
            @Override
            protected Future<Long> execute(RedisAsyncConnection<Object, V> async)
            {
    ...
    public V remove(final int index)
    {
        ...
        return getConnectionManager().write(new SyncOperation<Object, V>()
        {
            @Override
        ...
        for (int i = 1; i <= to; i++)
        {
            ...
            List<Object> range = getConnectionManager().read(new ResultOperation<List<Object>, Object>()
            {
                ...
                protected Future<List<Object>> execute(
                        RedisAsyncConnection<Object, Object> async)
                {
                    return async.lrange(getName(), startIndex, size - (j - 1)
                            * batchSize);
                }
            });
