...
public class RedisSet<V> extends RedisExpirable implements RSet<V>
{
    ...
    public RedisSet(ConnectionManager connectionManager, String name)
    {
    ...
    {
        return getConnectionManager().read(new ResultOperation<Boolean, Object>()
        {
            @Override
    ...
    {
        return getConnectionManager().read(new ResultOperation<ListScanResult<V>, V>()
        {
            @Override
            public Future<ListScanResult<V>> execute(
    ...
    {
        Set<V> res = getConnectionManager().read(new ResultOperation<Set<V>, V>()
        {
            @Override
    ...
    {
        return getConnectionManager().get(addAsync(e));
    }
    
    @Override
    public Future<Boolean> addAsync(final V e)
    {
        return getConnectionManager().writeAsync(new AsyncOperation<V, Boolean>()
        {
            @Override
            public void execute(final Promise<Boolean> promise,
                    RedisAsyncConnection<Object, V> async)
            {
                async.sadd(getName(), e)
                        .addListener(new OperationListener<V, Boolean, Long>(
                                promise, async, this)
                        {
                            @Override
    ...
    {
        return getConnectionManager().get(removeAsync((V) value));
    }
    ...
    {
        Long res = getConnectionManager().write(new ResultOperation<Long, Object>()
        {
            @Override
