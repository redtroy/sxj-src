...
public class RedisMap<K, V> extends RedisExpirable implements RMap<K, V>
{
    ...
    public RedisMap(ConnectionManager connectionManager, String name)
    {
    ...
    public int size()
    {
        return getConnectionManager().read(new ResultOperation<Long, V>()
        {
            @Override
    ...
    {
        return getConnectionManager().read(new ResultOperation<Boolean, V>()
        {
            @Override
    ...
    {
        return getConnectionManager().get(getAsync((K) key));
    }
    
    @Override
    public V put(K key, V value)
    {
        return getConnectionManager().get(putAsync(key, value));
    }
    
    @Override
    public V remove(Object key)
    {
        return getConnectionManager().get(removeAsync((K) key));
    }
    ...
    public Set<K> keySet()
    {
        return (Set<K>) getConnectionManager().read(new ResultOperation<Set<Object>, V>()
        {
            @Override
            public Future<Set<Object>> execute(
                    RedisAsyncConnection<Object, V> async)
            {
    ...
    {
        return getConnectionManager().read(new ResultOperation<List<V>, V>()
        {
            @Override
            public Future<List<V>> execute(RedisAsyncConnection<Object, V> async)
            {
    ...
    {
        Map<Object, Object> map = getConnectionManager().read(new ResultOperation<Map<Object, Object>, Object>()
        {
            @Override
            public Future<Map<Object, Object>> execute(
                    RedisAsyncConnection<Object, Object> async)
            {
    ...
    public V putIfAbsent(final K key, final V value)
    {
        ...
        return getConnectionManager().write(new AsyncOperation<V, V>()
        {
            @Override
            public void execute(final Promise<V> promise,
                    final RedisAsyncConnection<Object, V> async)
            {
                final AsyncOperation<V, V> timeoutCallback = this;
                async.hsetnx(getName(), key, value)
                        .addListener(new OperationListener<V, V, Boolean>(
                                promise, async, timeoutCallback)
                        {
                                    ...
                                    Future<Boolean> future) throws Exception
                            {
                                ...
                                async.hget(getName(), key)
                                        .addListener(new OperationListener<V, V, V>(
                                                promise, async, timeoutCallback)
                                        {
                                            @Override
                                            public void onOperationComplete(
    ...
    private boolean isEquals(RedisConnection<Object, Object> connection,
    ...
    {
        return getConnectionManager().write(new SyncOperation<V, V>()
        {
            @Override
    ...
    public Future<V> putAsync(final K key, final V value)
    {
        ...
        return getConnectionManager().writeAsync(new AsyncOperation<V, V>()
        {
            @Override
            public void execute(final Promise<V> promise,
                    RedisAsyncConnection<Object, V> async)
    ...
    {
        async.watch(getName()).addListener(new OperationListener<V, V, String>(
                promise, async, timeoutCallback)
        {
            @Override
                    ...
                    throws Exception
            {
                ...
                async.hget(getName(), key)
                        .addListener(new OperationListener<V, V, V>(promise,
                                async, timeoutCallback)
                        {
                            @Override
                            public void onOperationComplete(Future<V> future)
                                    throws Exception
                            {
                                
                                final V prev = future.get();
                                async.multi()
                                        .addListener(new OperationListener<V, V, String>(
                                                promise, async, timeoutCallback)
                                        {
                                            @Override
                                            public void onOperationComplete(
                                                ...
                                                async.hset(getName(),
                                                                ...
                                                                timeoutCallback)
                                                        {
                                                                    ...
                                                                    throws Exception
                                                            {
                                                                ...
                                                                async.exec()
                                                                        .addListener(new OperationListener<V, V, List<Object>>(
                                                                                promise,
                                                                                async,
                                                                                timeoutCallback)
                                                                        {
                                                ...
                                                promise, async, timeoutCallback)
                                        {
                                                    ...
                                                    throws Exception
                                            {
                                                ...
                                                async.hdel(getName(), key)
                                                        .addListener(new OperationListener<V, V, Long>(
                                                                promise, async,
                                                                timeoutCallback)
                                                        {
                                                            @Override
    ...
    {
        return getConnectionManager().writeAsync(new ResultOperation<Boolean, V>()
        {
            @Override
            public Future<Boolean> execute(RedisAsyncConnection<Object, V> async)
            {
    ...
    {
        return getConnectionManager().get(fastPutAsync(key, value));
    }
    ...
    {
        return getConnectionManager().get(fastRemoveAsync(keys));
    }
    
    private MapScanResult<Object, V> scanIterator(final long startPos)
    {
        return getConnectionManager().read(new ResultOperation<MapScanResult<Object, V>, V>()
        {
            @Override
            public Future<MapScanResult<Object, V>> execute(
                    RedisAsyncConnection<Object, V> async)
            {
    ...
    private Iterator<Map.Entry<K, V>> iterator()
    {
        return new Iterator<Map.Entry<K, V>>()
        {
            ...
            public void remove()
            {
                ...
                hasNext();
                iter.remove();
                RedisMap.this.fastRemove(value.getKey());
                removeExecuted = true;
            }
    ...
    {
        String res = getConnectionManager().write(new ResultOperation<String, V>()
        {
                    ...
                    RedisAsyncConnection<Object, V> async)
            {
                ...
                return async.hincrbyfloat(getName(),
                        key,
                        new BigDecimal(val.toString()).toPlainString());
            }
        });
