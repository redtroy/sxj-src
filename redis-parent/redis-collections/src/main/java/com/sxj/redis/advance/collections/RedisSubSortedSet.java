...
class RedisSubSortedSet<V> implements SortedSet<V>
{
    
    private ConnectionManager connectionManager;
    
    private RedisSortedSet<V> redissonSortedSet;
    
    private V headValue;
    
    private V tailValue;
    
    public RedisSubSortedSet(RedisSortedSet<V> redissonSortedSet,
            ConnectionManager connectionManager, V headValue, V tailValue)
    {
        super();
    ...
    public int size()
    {
        ...
        try
        {
            ...
            return connection.zcount(redissonSortedSet.getName(),
                    headScore,
                    tailScore).intValue();
        }
        finally
    ...
    {
        BinarySearchResult<V> res = redissonSortedSet.binarySearch(tailValue,
                connection);
        if (res.getIndex() < 0)
        {
            return 0;
    ...
    {
        BinarySearchResult<V> res = redissonSortedSet.binarySearch(headValue,
                connection);
        if (res.getIndex() < 0)
        {
            return 0;
    ...
    public boolean contains(Object o)
    {
        ...
        try
        {
            ...
            BinarySearchResult<V> res = redissonSortedSet.binarySearch((V) o,
                    connection);
    ...
    public Object[] toArray()
    {
        ...
        try
        {
            double headScore = getHeadScore(connection);
            double tailScore = getTailScore(connection);
            return connection.zrangebyscore(redissonSortedSet.getName(),
                    headScore,
                    tailScore).toArray();
        }
        finally
    ...
    public SortedSet<V> subSet(V fromElement, V toElement)
    {
        ...
        return new RedisSubSortedSet<V>(redissonSortedSet, connectionManager,
