...
import com.sxj.redis.RedisMovedException;
...
public class RedisSortedSet<V> extends RedisObject implements RSortedSet<V>
{
    ...
    private static class NaturalComparator<V> implements Comparator<V>,
            Serializable
    {
    ...
    public RedisSortedSet(ConnectionManager connectionManager, String name)
    {
    ...
    {
        getConnectionManager().read(new SyncOperation<V, Void>()
        {
            @Override
    ...
    private void loadComparator(RedisConnection<Object, ?> connection)
    {
        try
        {
            ...
            if (comparatorSign != null)
            {
                ...
                {
                    throw new IllegalStateException("Local class signature of "
                            + className
                            + " differs from used by this SortedSet!");
                }
    ...
    private static String calcClassSign(String name)
    {
        ...
        {
            throw new IllegalStateException("Can't calculate sign of " + name,
                    e);
        }
    }
    ...
    {
        return getConnectionManager().read(new SyncOperation<V, Boolean>()
        {
            @Override
    ...
    public Iterator<V> iterator()
    {
        ...
        return new Iterator<V>()
        {
            ...
            public V next()
            {
                ...
                return RedisSortedSet.this.get(currentIndex);
            }
            ...
            public void remove()
            {
                ...
                RedisSortedSet.this.remove(currentIndex);
                currentIndex--;
                removeExecuted = true;
            }
    ...
    {
        getConnectionManager().write(new SyncOperation<Object, V>()
        {
            @Override
            public V execute(RedisConnection<Object, Object> conn)
            {
                ...
                {
                    conn.watch(getName());
                    List<Object> tail = conn.lrange(getName(),
                            index + 1,
                            size());
                    conn.multi();
    ...
    {
        return getConnectionManager().read(new ResultOperation<V, V>()
        {
            @Override
    ...
    {
        List<V> res = getConnectionManager().read(new ResultOperation<List<V>, V>()
        {
            @Override
    ...
    private Long getCurrentVersion(
            RedisConnection<Object, Object> simpleConnection)
    {
    ...
    {
        return getConnectionManager().write(new SyncOperation<V, Boolean>()
        {
            @Override
            public Boolean execute(RedisConnection<Object, V> conn)
            {
    ...
    boolean remove(Object value, RedisConnection<Object, V> conn)
    {
        while (true)
        {
            ...
            List<Object> tail = сonnection.lrange(getName(),
                    res.getIndex() + 1,
                    size());
            сonnection.multi();
    ...
    public V first()
    {
        V res = getConnectionManager().read(new ResultOperation<V, V>()
        {
            @Override
            protected Future<V> execute(RedisAsyncConnection<Object, V> async)
            {
    ...
    {
        return getConnectionManager().write(new SyncOperation<String, Boolean>()
        {
            @Override
            public Boolean execute(RedisConnection<Object, String> connection)
            {
                ...
                String comparatorSign = className + ":"
                        + calcClassSign(className);
                connection.set(getComparatorKeyName(), comparatorSign);
                List<Object> res = connection.exec();
                if (res.size() == 1)
                {
                    RedisSortedSet.this.comparator = comparator;
                    return true;
                }
                return false;
            }
     ...
     * @return
     */
    private BinarySearchResult<V> binarySearch(V value,
            RedisConnection<Object, V> connection, int lowerIndex,
            int upperIndex)
    {
    ...
    double score(V value, RedisConnection<Object, V> connection, int indexDiff,
            boolean tail)
    {
        return -1;
        //        BinarySearchResult<V> res = binarySearch(value, connection);
