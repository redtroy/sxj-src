...
public class RedisConnection<K, V>
{
    ...
    {
        long timeout2 = (timeout == 0 ? Long.MAX_VALUE : max(timeout,
                unit.toSeconds(this.timeout)));
    ...
    public V brpoplpush(long timeout, K source, K destination)
    {
        long timeout2 = (timeout == 0 ? Long.MAX_VALUE : max(timeout,
                unit.toSeconds(this.timeout)));
        return await(c.brpoplpush(timeout, source, destination),
                timeout2,
                SECONDS);
    }
    ...
    public List<V> zrangebyscore(K key, double min, double max, long offset,
            long count)
    {
        return await(c.zrangebyscore(key, min, max, offset, count));
    }
    
    public List<V> zrangebyscore(K key, String min, String max, long offset,
            long count)
    {
        return await(c.zrangebyscore(key, min, max, offset, count));
    }
    ...
    public List<ScoredValue<V>> zrangebyscoreWithScores(K key, String min,
            String max)
    {
        return await(c.zrangebyscoreWithScores(key, min, max));
    }
    ...
    public List<V> zrevrangebyscore(K key, double max, double min, long offset,
            long count)
    {
        return await(c.zrevrangebyscore(key, max, min, offset, count));
    }
    
    public List<V> zrevrangebyscore(K key, String max, String min, long offset,
            long count)
    {
        return await(c.zrevrangebyscore(key, max, min, offset, count));
    }
    ...
    public List<ScoredValue<V>> zrevrangebyscoreWithScores(K key, String max,
            String min)
    {
        return await(c.zrevrangebyscoreWithScores(key, max, min));
    }
