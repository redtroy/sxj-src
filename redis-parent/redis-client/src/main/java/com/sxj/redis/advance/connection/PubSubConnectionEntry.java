...
public class PubSubConnectionEntry
{
    ...
    public PubSubConnectionEntry(RedisPubSubConnection conn,
            int subscriptionsPerConnection)
    {
        super();
        this.conn = conn;
        this.subscriptionsPerConnection = subscriptionsPerConnection;
        this.subscribedChannelsAmount = new Semaphore(
    ...
    public void addListener(String channelName, RedisPubSubListener listener)
    {
        ...
        if (queue == null)
        {
            ...
            Queue<RedisPubSubListener> oldQueue = channelListeners.putIfAbsent(channelName,
                    queue);
            if (oldQueue != null)
            {
                queue = oldQueue;
        ...
        synchronized (queue)
        {
            if (queue.remove(listener))
            {
                channelListeners.remove(channelName,
                        new ConcurrentLinkedQueue<RedisPubSubListener>());
            }
        }
