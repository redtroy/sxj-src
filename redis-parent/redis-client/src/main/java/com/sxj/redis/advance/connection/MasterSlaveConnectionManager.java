...
import com.sxj.redis.RedisTimeoutException;
...
import com.sxj.redis.pubsub.RedisPubSubListener;
...
public class MasterSlaveConnectionManager implements ConnectionManager
{
    ...
    public MasterSlaveConnectionManager(MasterSlaveServersConfig cfg,
            Config config)
    {
        init(cfg, config);
    }
            ...
            final RedisConnection conn)
    {
        return new FutureListener<T>()
        {
            ...
            public void operationComplete(
                    io.netty.util.concurrent.Future<T> future) throws Exception
            {
    ...
    private <V, T> void writeAllAsync(final int slot,
            final AsyncOperation<V, T> asyncOperation,
            final AtomicInteger counter, final Promise<T> mainPromise,
            final int attempt)
    {
    ...
    private <V, T> void writeAsync(final int slot,
            final AsyncOperation<V, T> asyncOperation,
    ...
    private <V, T> void readAsync(final int slot,
            final AsyncOperation<V, T> asyncOperation,
            final Promise<T> mainPromise, final int attempt)
    {
        final Promise<T> promise = getGroup().next().newPromise();
    ...
    public <K, V> PubSubConnectionEntry subscribe(String channelName)
    {
        ...
        Set<PubSubConnectionEntry> entries = new HashSet<PubSubConnectionEntry>(
                name2PubSubConnection.values());
        for (PubSubConnectionEntry entry : entries)
        {
            if (entry.tryAcquire())
            {
                PubSubConnectionEntry oldEntry = name2PubSubConnection.putIfAbsent(channelName,
                        entry);
                if (oldEntry != null)
                {
                    entry.release();
        ...
        PubSubConnectionEntry entry = new PubSubConnectionEntry(conn,
                config.getSubscriptionsPerConnection());
        entry.tryAcquire();
        PubSubConnectionEntry oldEntry = name2PubSubConnection.putIfAbsent(channelName,
                entry);
        if (oldEntry != null)
    ...
    {
        Collection<RedisPubSubConnection> allPubSubConnections = getEntry().slaveDown(host,
                port);
            ...
            for (RedisPubSubConnection redisPubSubConnection : allPubSubConnections)
            {
                ...
                synchronized (entry)
                {
                    ...
                    if (!listeners.isEmpty())
                    {
                        ...
                        {
                            newEntry.addListener(channelName,
                                    redisPubSubListener);
                        }
                    }
                }
    ...
    protected void returnSubscribeConnection(int slot,
