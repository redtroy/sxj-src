...
import com.sxj.redis.codec.Utf8StringCodec;
import com.sxj.redis.protocol.Command;
import com.sxj.redis.protocol.CommandHandler;
import com.sxj.redis.protocol.ConnectionWatchdog;
import com.sxj.redis.pubsub.PubSubCommandHandler;
import com.sxj.redis.pubsub.RedisPubSubConnection;

/**
 * A scalable thread-safe <a href="http://redis.io/">Redis</a> client. Multiple
 * threads may share one connection provided they avoid blocking and
...
public class RedisCli {
	...
	/**
	 * Create a new client that connects to the supplied host on the default
	 * port.
	 *
	 * @param host
	 *            Server hostname.
	 ...
	 * Connection attempts and non-blocking commands will
	...
	public RedisCli(EventLoopGroup group, String host, int port, int timeout) {
		...
		bootstrap = new Bootstrap().channel(NioSocketChannel.class)
				.group(group).remoteAddress(addr);
	...
	/**
	 * Set the default timeout for {@link RedisConnection connections} created
	 * by this client. The timeout applies to connection attempts and
	...
	/**
	 * Open a new synchronous connection to the redis server that treats keys
	...
	/**
	 * Open a new asynchronous connection to the redis server that treats keys
	 * and values as UTF-8 strings.
	 *
	 * @return A new connection.
	 */
	...
	/**
	 * Open a new pub/sub connection to the redis server that treats keys and
	 ...
	 *
	 * @param codec
	 *            Use this codec to encode/decode keys and values.
	 *
	 * @return A new connection.
	 */
	...
	public void shutdown() {
		for (Channel c : channels) {
			...
			RedisAsyncConnection<?, ?> connection = pipeline
					.get(RedisAsyncConnection.class);
			connection.close();
		}
