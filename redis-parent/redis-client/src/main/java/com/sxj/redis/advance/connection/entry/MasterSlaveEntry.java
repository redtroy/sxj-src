...
import com.sxj.redis.advance.connection.balancer.LoadBalancer;
...
public class MasterSlaveEntry
{
    ...
    public MasterSlaveEntry(RedisCodec codec, EventLoopGroup group,
            MasterSlaveServersConfig config)
    {
        ...
        {
            RedisCli client = new RedisCli(group, address.getHost(),
                    address.getPort(), config.getTimeout());
            SubscribesConnectionEntry entry = new SubscribesConnectionEntry(
                    client, config.getSlaveConnectionPoolSize(),
                    config.getSlaveSubscriptionConnectionPoolSize());
            slaveBalancer.add(entry);
        }
        if (config.getSlaveAddresses().size() > 1)
        {
            slaveDown(config.getMasterAddress().getHost(),
                    config.getMasterAddress().getPort());
        }
        
        setupMasterEntry(config.getMasterAddress().getHost(),
                config.getMasterAddress().getPort());
    }
    
    public void setupMasterEntry(String host, int port)
    {
        RedisCli masterClient = new RedisCli(group, host, port,
                config.getTimeout());
        masterEntry = new ConnectionEntry(masterClient,
    ...
    {
        slaveDown(masterEntry.getClient().getAddr().getHostName(),
                masterEntry.getClient().getAddr().getPort());
        
        RedisCli client = new RedisCli(group, host, port, config.getTimeout());
