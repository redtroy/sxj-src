...
public class LogisticsRefServiceImpl implements ILogisticsRefService
{
    ...
    public void update(LogisticsRefEntity model) throws ServiceException
    {
        ...
        {
            refDao.update(model);
            CometServiceImpl.subCount(RfidChannel.RFID_MANAGER_LOGISTICS_MESSGAGE_REF);
            RfidChannel.initTopic()
                    .publish(RfidChannel.RFID_MANAGER_LOGISTICS_MESSGAGE_REF);
        }
        catch (Exception e)
        {
            SxjLogger.error(e.getMessage(), e, this.getClass());
    ...
    public void add(LogisticsRefEntity model) throws ServiceException
    {
        try
        {
            ...
            if (model != null)
            {
                ...
                refDao.add(model);
                CometServiceImpl.takeCount(RfidChannel.RFID_MANAGER_LOGISTICS_MESSGAGE_REF);
                RfidChannel.initTopic()
                        .publish(RfidChannel.RFID_MANAGER_LOGISTICS_MESSGAGE_REF);
            }
        }
