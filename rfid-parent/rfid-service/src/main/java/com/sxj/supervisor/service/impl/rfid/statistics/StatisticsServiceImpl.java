...
public class StatisticsServiceImpl implements IStatisticsService
{
            ...
            throws ServiceException
    {
        try
        {
            ...
            // 门窗标签销售量
            StatisticsItemModel windowSaleItem = statisticsWindowSale(startDate,
                    endDate);
            ...
            // 门窗标签销售额
            StatisticsItemModel windowSaleAmountItem = statisticsWindowAmountSale(startDate,
                    endDate);
            model.setWindowSaleAmountList(windowSaleAmountItem);
            
            StatisticsItemModel batchSaleAmountItem = statisticsBatchAmountSale(startDate,
                    endDate);
            model.setBatchSaleAmountList(batchSaleAmountItem);
            
            // 门窗采购额
            StatisticsItemModel windowPurchaseAmountItem = statisticsWindowPurchaseAmount(startDate,
                    endDate);
            model.setWindowPurchaseAmountList(windowPurchaseAmountItem);
            // 批次采购额
            StatisticsItemModel batchPurchaseAmountItem = statisticsWindowBatchAmount(startDate,
                    endDate);
            model.setBatchPurchaseAmountList(batchPurchaseAmountItem);
            
            // 预计利润
            StatisticsItemModel expectProfitList = statisticsApplyAmount(startDate,
                    endDate);
            model.setExpectProfitList(expectProfitList);
            // 实际利润
            StatisticsItemModel realityProfitList = statisticsRealityProfit(startDate,
                    endDate);
            ...
            throws ServiceException
    {
        ...
        for (Iterator<RfidApplicationEntity> iterator = appList.iterator(); iterator.hasNext();)
        {
            RfidApplicationEntity rfidApplicationEntity = iterator.next();
            appCount = appCount + rfidApplicationEntity.getCount();
            applyDateList.add(DateTimeUtils.formatPageDate(rfidApplicationEntity.getApplyDate()));
            applyCountList.add(new Double(rfidApplicationEntity.getCount() + ""));
        }
            ...
            String endDate) throws ServiceException
    {
        ...
        List<RfidPurchaseEntity> purchaseList = purchaseService.queryPurchase(purchaseQuery);
        ...
        for (RfidPurchaseEntity rfidPurchaseEntity : purchaseList)
        {
            ...
            purchaseDateList.add(DateTimeUtils.formatPageDate(rfidPurchaseEntity.getPurchaseDate()));
            purchaseCountList.add(new Double(rfidPurchaseEntity.getCount() + ""));
        }
