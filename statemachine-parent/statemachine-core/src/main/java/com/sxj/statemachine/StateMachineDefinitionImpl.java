...
public class StateMachineDefinitionImpl<S extends Enum<?>> implements
	...
	void defineTransition(Transition transition, final Method method,
		...
		if (method != null)
			this.defineTransition(transition.source(), transition.event(),
					transition.target(), new TransitionController() {
	...
	private void printTransitionsForState(State state, StringBuilder sb) {
		...
		if (state.getExitStateController() != null)
			sb.append("<ExitState state=\"").append(state.getName())
		...
		for (String event : txs.keySet()) {
			...
			sb.append("<Transition ").append("source=\"")
					.append(state.getName()).append("\" ").append("event=\"")
					.append(event).append("\" ").append("target=\"")
					.append(target.getState()).append("\"").append(" />")
					.append(NEWLINE);
		}

		if (state.getEnterStateController() != null)
			sb.append("<EnterState state=\"").append(state.getName())
					.append("\" />");
	...
	private class State {
		...
		public void setTransitionController(String event, String target,
			...
			if (!transitions.containsKey(event))
				transitions
						.put(event, new TransitionTarget(target, controller));
		}
